...
class keenlab_tencent_pwn_plugin:
	def __init__(self,loop,collection,lock):
		...
		self.loop = loop
		self.rss = 'https://keenlab.tencent.com/zh/atom.xml'
				...
				if (article_time.year == format_time.year) and (article_time.month == format_time.month) and (article_time.day == format_time.day):
					...
					try:
						...
						if self.collection.count_documents({'link':entrie['link']}) < 1:
							...
							logger.info('[Pwn] {} {}'.format(entrie['title'],entrie['link']))
					finally:
						self.lock.release()
		else:
...
if __name__ == '__main__':
	...
	class_name = keenlab_tencent_pwn_plugin(loop,collection,lock)
