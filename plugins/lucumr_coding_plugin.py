# -*- coding: utf-8 -*-
# @Author: p1g3
# @Date:   2020-01-30 18:01:05
# @Last Modified by:   p1g3
# @Last Modified time: 2020-01-30 22:39:37
...
class lucumr_coding_plugin:
	def __init__(self,loop,collection,lock):
		...
		self.loop = loop
		self.rss = 'https://lucumr.pocoo.org/feed.atom'
				...
				if (article_time.year == format_time.year) and (article_time.month == format_time.month) and (article_time.day == format_time.day):
					...
					try:
						...
						if self.collection.count_documents({'link':entrie['link']}) < 1:
							...
							logger.info('[Coding] {} {}'.format(entrie['title'],entrie['link']))
					finally:
						self.lock.release()
		else:
...
# 	class_name = a_web_plugin(loop,collection,lock)
